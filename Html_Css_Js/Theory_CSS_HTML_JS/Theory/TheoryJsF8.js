...
4.Replace:
 ...
 replace(/JS/g,'Javascript') -> thay tat cac JS trong chuoi -> Javascript
5.Split:
...
->javascript,PHP,Ruby.
1.1 Join:console.log(language.join(' - '))  (as split syntax)
->javascript - PHP - Ruby.
...
Ex: var result = course.every(function(object,index,originArray){
 return object.coin === 0; gap 1 phan tu co coin != 0 -> return false
})
console.log(result);
...
// bai tap vd : tim cac mon the thao co like > 5
var sport = [
 {
   name:"soccer",
   like:666,
...
// vd: khoi tao phong thuc reduce 2 cho mang
// function reduce2(c,r) {
//   let i = 0
//   if(arguments.length < 2) {
//     i = 1;
//     r = this[0];
...
// }
// // tao ham reduce 2
// Array.prototype.r = reduce2

// a = [1,2,3,4,5]
// console.log(a.r(function(total,number, index, arr){
//   return total+number;
// },0))
...
//   var i=0;
//   var arraylength = this.length;
//   if(arguments.size <2) // khong truyen initial value -> doi so < 2
//   {
//     result = this[0]; // bien tich tru phan tu thu 1 cua mang
//     i = 1; // bat dau tu phan tu thu 2
//   }
//   for (var i=0;i<arraylength;i++){
...
// - syntax  Promise
// var newPro = new Promise( function (resolve, reject) { logic for resolve(s), reject (e)})
// newPro
//   .then( function(s) {} )  -> resolve run
//   .catch ( function(e) {} ) -> reject run
//   .finally( function() {} ) -> run neu resolve hay reject chay

// - concept: da dc tao truoc bang Object constructor -> chi sd
...
// vd2: xu li bat dong bo
// const wait = (ms) => {
//     return new Promise((resolve, reject) => {
//         setTimeout(() => {
//             console.log(`wait ${ms} done!`);
//         }, ms);
//     });
// };
...
// Ví dụ 3: Kết hợp nhiều Promise
// const promise1 = Promise.resolve("Promise 1 hoàn thành");
// const promise2 = Promise.resolve("Promise 2 hoàn thành");

// Promise.all([promise1, promise2])
//     .then((results) => {
//         console.log(results); // Kết quả là một mảng chứa kết quả của các promise
//     })
//     .catch((error) => {
//         console.error(error); // Nếu bất kỳ promise nào bị lỗi
//     });

// out: [ 'Promise 1 hoàn thành', 'Promise 2 hoàn thành' ]

// Ví dụ 4: Sử dụng Promise.race
...
//   .then(result => {
//     console.log(result); // Kết quả của promise hoàn thành nhanh nhất
//   });
...
//         setTimeout(resolve(`you wait ${ms} milisecond`), ms);
//     });
// }
...
//         id: 2,
//         user_id: 2,
//         content: "Hi anh em, toi la Hao",
//     },
//     {
//         id: 5,
//         user_id: 5,
//         content: "Helo bros, my name is tai cali :v",
//     },
//     {
//         id: 4,
//         user_id: 4,
//         content: "Helo bros, my name is tung cali :v",
//     },
//     {
//         id: 7,
//         user_id: 7,
//         content: "Helo bros, my name is thuy",
//     },
// ];

// * code without promise: chuong
// list id comment
// let comment_id = [];
// comments.forEach((v) => {
//     comment_id.push(v.user_id);
// });

// // list id users duplicate id in comments
// let Users = [];
// users.forEach((v) => {
//     if (comment_id.includes(v.id)) {
//         Users.push(v);
//     }
// });
...
// var htmls = ""
// Users.forEach((v) => {
//     let o = comments.find((c) => {
//         return c.user_id == v.id;
//     });
//     htmls += `<li> ${v.name} : ${o.content} </li> `
// });
// var x = document.getElementById('comment-block')
// x.innerHTML = htmls

// * code with promise: chuong
// function getComments() {
//     return new Promise((resolve) => {
//         // gia su mang nghen 1s
//         setTimeout(resolve(comments), 1000);
//     });
// }

// // get id in comments
// function getIdInComment() {
//     let list_id_comment = [];
//     getComments().then((data) => {
//         data.forEach((v) => {
//             list_id_comment.push(v.user_id);
//         });
//     });
//     return new Promise((resolve) => {
//         resolve(list_id_comment);
//     });
// }

// // list user  duplicates id in comments
// function getUserByIdInComments() {
//     let list_user_id = [];
//     getIdInComment().then((data) => {
//         users.forEach((v) => {
...
//     return new Promise((resolve) => {
//         resolve(list_user_id);
//     });
// }

// function displayDataOnScreen() {
//     let htmls = "";
//     getUserByIdInComments().then((data) => {
//         data.forEach((v) => {
//             var o = comments.find((c) => {
//                 return c.user_id == v.id;
//             });
//             htmls += `<li> ${v.name} : ${o.content} </li>`;
//         });
//         let x = document.getElementById('comment-block');
//         x.innerHTML = htmls;
//     });
// }
// displayDataOnScreen();
...
// 3/ tu user_id(comments) lay user(users) tuong ung [user_id === id]
...
//   return new Promise(function(resolve){
// filter -> duyet qua tung pt -> lay pt co id trong danh sach id(userIDs)
//     var result = users.filter(function(user){
//       return userIDs.includes(user.id); // return true / false -> filter lay cac phan tu return true(id trung)
//     });
//     setTimeout(function(){
//       resolve(result);
//     }, 1000);
...
//     });
//     // return Promise
//     return getUsersByIDs(userIDs) // return list user (mang users) thong qua user_id cua mang comments
//     .then(function(users){
//       return{
...
//     })
//   })
//   // lay du lieu return tu .then truoc do gan data = object bao gom 2 list : list users va comments
//   .then(function(data){
...
//   .then(function(data){
//     return data.json(); // data.parse() : json -> js
//   })

//   xu li du lieu de render ra man hinh: posts = lay du lieu return truoc do cua promise data.json(json -> js)
//   .then(function(posts) {
...
// Update -> PUT : raw -> URL../id -> update key,value -> send
// get: nhan data
// post: body->xwww-form-urlencoded : khoi tao object moi (key:value)

// Example CRUD with Postman
...
//     };
//     fetchData(api, options, callback);
// }
...
//         body: JSON.stringify(data),
//     };
//     fetchData(`${api}/${id}`, options, callback);
// }
...
//         headers: { 'Content-Type': 'application/json' },
//     };
//     fetchData(`${api}/${id}`, options, () => {
//         getCourses(renderCourses); // Làm mới danh sách sau khi xóa
//     });
// }
...
//             <h2>${course.name}</h2>
//             <p>${course.des}</p>
//             <button onclick="deleteCourse(${course.id})">Del</button>
//             <button onclick="loadUpdateForm(${course.id}, '${course.name}', '${course.des}')">Update</button>
//         </li>
//     `);
//     document.getElementById("list-block").innerHTML = htmls.join("");
// }
...
//         const isUpdating = buttonCreate.dataset.updateId; // id

//         if (isUpdating) {
//             // update
//             const data = { name, des };
//             updateCourse(isUpdating, data, () => {
//                 getCourses(renderCourses);
//                 resetForm();
//             });
//         } else {
...
//                     ? Math.max(...courses.map(course => Number(course.id)))  //... Dùng để "trải" các phần tử trong mảng thành các tham số riêng lẻ cho hàm.
//                     : 1; // Lấy ID lớn nhất hoặc 0 nếu danh sách rỗng
//                 const newId = lastId + 1;

//                 // Tạo mới khóa học với ID tự động
//                 const data = { id: String(newId), name, des };
//                 createCourse(data, () => {
//                     getCourses(renderCourses); // Làm mới danh sách sau khi thêm
//                 });
//             });
//         }
//     });
...
// // Tải form cập nhật
// function loadUpdateForm(id, name, des) {
//     document.querySelector('input[name="name"]').value = name;
//     document.querySelector('input[name="des"]').value = des;
//     const buttonCreate = document.getElementById("btn");
//     buttonCreate.innerText = "Update Course";
//     buttonCreate.dataset.updateId = id; // Gán ID để xử lý cập nhật (tạo thuộc tính vói giá trị là id trong nút)
// }
...
//     const buttonCreate = document.getElementById("btn");
//     buttonCreate.innerText = "Create Course";
//     delete buttonCreate.dataset.updateId; // Xóa trạng thái cập nhật
// }
...
function fetchData(api, options, callback) {
    fetch(api, options)
        .then(function (responsive) {
            return responsive.json(); // json -> js
        })
        .then(callback) // call back = function(data)
        .catch(function (err) {
            console.error(err);
        });
}
...
function deleteCourse(id) {
    ...
    fetchData(`${api}/${id}`, options, function () {
        getCourses(renderCourse); // Làm mới danh sách sau khi xóa
    });
}
...
function renderCourse(courses) {
    ...
    <button onclick="loadForm(${course.id},' ${course.name}', '${course.des}')"> Update </button>
    `;
    });
    let block = document.getElementById("list-block");
...
function handleForm() {
    ...
    btn.addEventListener("click", function () {
        ...
        // Kiểm tra nếu tên hoặc mô tả trống
        if (!name || !des) {
            alert("Vui lòng nhập đầy đủ thông tin!");
            return;
        }

        const isUpdating = btn.dataset.updateId;

        if (isUpdating) {
            const data = { name: name, des: des };
            updateCourse(isUpdating, data, function () {
                getCourses(renderCourse);
                resetForm();
            });
        } else {
            getCourses(function (courses) {
                ...
                let id = 1;
                let list_id = courses.map(function (course) {
                    return Number(course.id);
                }); // list id
                if (len > 0) {
                    let current_id = Math.max(...list_id); // max id
                    id = current_id + 1; // new id
                }

                const new_data = { id: String(id), name: name, des: des };
...
function loadForm(id, name, des) {
    let btn = document.getElementById("btn");
    document.querySelector("input[name='name']").value = name;
    document.querySelector("input[name='des']").value = des;
    btn.innerHTML = "Update";
    btn.dataset.updateId = id;
}
